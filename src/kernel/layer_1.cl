...
// MACRO: N1_FILTER_COUNT filter count for first layer
...
void main(__read_only __global float* source,
					...
					uint f1,
					int w, int h){
	const int half_f1 = f1 / 2;
	const int2 padding = {half_f1, half_f1}; // left top corner
	const int2 size_source = {w, h};
	...
	if(pos.x >= padding.x && pos.x <= br.x &&
		...
		// (result cube's first 2 dimensions(w-f1+1, h-f1+1) are smaller then source image's(w*h))
