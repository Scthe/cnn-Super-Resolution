...
__kernel void main(__read_only __global float* deltas,       //
  ...
  if (id < weights_size) {
    ...
    for (size_t i = 0; i < layer_out_w; i++) {
      for (size_t j = 0; j < layer_out_h; j++) {
        // (1) delta[i,j,n](l)
        int2 pos = {j, i};
        int idx = ((pos.y * layer_out_w) + pos.x) * n_current_filter_cnt;
        float delta = deltas[idx + n];
        grad_b += delta;

        // (2) layer_input[i+b,j+a,k]
        // NOTE: we are subtracting here! <- nope ?
        // int2 prev_layer_pos = {pos.x - dx, pos.y - dy};
        int2 prev_layer_pos = {pos.x + dx, pos.y + dy};
        int prev_layer_idx = ((prev_layer_pos.y * input_w) + prev_layer_pos.x) *
                             n_prev_filter_cnt;

        // if (prev_layer_pos.x > 0 && prev_layer_pos.y > 0) {
